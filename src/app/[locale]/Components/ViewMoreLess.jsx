  ...
  return (
    <div className="grid gap-4">
      ...
      <ul className="grid gap-3 [&>*]:font-light [&>a]:hover:text-primary [&>*]:transition-all [&>*]:text-[14px]">
        {items && items.slice(0, showAll ? items.length : limit).map((item, index) => (
          <li key={index}>
      ...
      {items.length > limit && (
        ...
        >
          {showAll ? getTranslation(translation[0]?.translations, "View less", locale || 'en') :  getTranslation(translation[0]?.translations, "View all", locale || 'en')}
        </button>
      )}
    </div>
  );
