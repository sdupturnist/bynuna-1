...
export default async function ShippingPolicy({params: { locale }}) {


  let pageData = await fetch(
    `${apiUrl}wp-json/wp/v2/pages?slug=shipping-policy&lang=${locale || 'en'}`,
    {
      next: { revalidate: 60 },
    }
  );
...
export async function generateMetadata({ params, searchParams }, parent) {
  ...
  try {
    const page = await fetch(
      `${apiUrl}wp-json/wp/v2/pages?slug=shipping-policy&lang=en`,
